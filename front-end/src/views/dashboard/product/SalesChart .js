...
const SalesChart = () => {
    ...
    const fetchData = async () => {
        try {
            const averageResponse = await axios.get(`http://127.0.0.1:5000/api/average-sales-price?period=${period}`);
            const topSellingResponse = await axios.get(`http://127.0.0.1:5000/api/top-selling-products?period=${period}`);
