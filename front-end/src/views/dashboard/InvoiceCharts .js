...
const InvoiceCharts = () => {
  ...
  useEffect(() => {
    const fetchTotalRevenue = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/total-revenue');
    ...
    const fetchAverageInvoiceValue = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/average-invoice-value');
    ...
    const fetchInvoiceCount = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/invoice-count');
