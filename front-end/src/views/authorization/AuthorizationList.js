...
function AuthorizationList() {
  ...
  useEffect(() => {
    const fetchAuthorizations = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/addEmployeesToAuthorization', config);
  ...
  const updatestatusEmployesAutorisation = async (email_employe, updatedAuth) => {
    try {
      ...
      const response = await axios.put('http://127.0.0.1:5000/api/updatestatusEmployesAutorisation', dataToSend, config);
